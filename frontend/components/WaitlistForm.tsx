...
export default function WaitlistForm({ source = 'general', onClose }: WaitlistFormProps) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Extract a meaningful error message from Axios error responses when
      // available.  The backend returns an `error` object with a
      // `message` field.  Fall back to the native error message or a
      // generic message when nothing else is available.
      const message =
